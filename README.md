...
| `/야차 상대:@멤버 수위:순한맛|매운맛|불맛|지옥맛 조합:HL|BL|GL 횟수:1~3 상세:True` ||매운맛|불맛|지옥맛 횟수:1~3 상세:True` | 연령제한 채널 전용. 태그 풀 = booru-v5(단보루 성인 태그 ~900개·한글 번역, 24h 캐시 `data/booru/`, 실패 시 내장 풀). 대결→승자가 그려옴. 불맛↑ 공통/🔝탑(여성형 의상·신체 제외)/🔻바텀. `상세`=태그 뜻+연관 세부 태그. 페티시·미성년·수간·고어·배설 제외 |
| `/폭력 상대:@멤버 횟수:1~3 상세:True` | 연령제한 채널 전용. 폭력이다! 폭력! — 유혈(폭력성 분류)+비동의·강압 태그 전용 |
